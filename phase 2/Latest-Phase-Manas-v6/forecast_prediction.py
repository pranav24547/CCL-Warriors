"""
============================================================================
CISCO CFL PHASE 2 - COMPETITION FORECAST v6.0 (CRYSTAL CUTTHROAT II)
============================================================================
...
for p in products:
    ...
    # Signal 3: Q2 Seasonal Average (v6.0: replaces MA4 — MA4 dilutes seasonal signal)
    # MA4 mixes Q2+Q3+Q4+Q1, diluting the Q2 spike. Q2-only history is more relevant.
    ...
    if len(q2_nz) >= 3:
        ...
        q2_seasonal_fc = sum(act[-4:])/4  # fallback to MA4 only when no Q2 history
    ma4_fc = sum(act[-4:])/4  # kept for validation output only
    ...
    q2_avg_fc = q2_seasonal_fc  # same as structural signal 3 in v6.0
    ...
    # ============================================================
    # ONLY 3 truly independent signals (v6.0: Q2 seasonal avg replaces MA4)
    ind_signals = [v for v in [ratio_fc, yoy_fc, q2_seasonal_fc] if v > 0]
    ...
    if q1_drop > 0.25 and ratio_fc > 0:
        ...
        other_struct = [v for v in [yoy_fc, q2_seasonal_fc] if v > 0]
        if other_struct:
    ...
    # ============================================================
    # EXPERT-ANCHORED BLEND (v6.0: smooth linear interpolation)
    # ============================================================
    # Linear interpolation: maps accuracy [0.20, 0.95] -> weight [0.25, 0.85]
    # Replaces v5.0's discontinuous step function
    expert_weight = clamp(0.25 + (avg_expert_acc - 0.20) * (0.85 - 0.25) / (0.95 - 0.20), 0.25, 0.85)
...
L.append("  Version 6.0 - Crystal Cutthroat II (Research-Validated)")
...
L.append("  v6.0 CHANGES: Damped equal weights, no dominant expert rule, Q2 seasonal avg,")
L.append("                pattern-based overrides, smooth blend function, bias threshold 8%")
L.append("")
L.append("  Weighting: 60% equal + 40% acc^1 (shrink-toward-equal, per combination puzzle)")
L.append("  Structural: 3 independent signals (Q2/Q1 ratio, YoY Q2, Q2 Seasonal Avg)")
L.append("  Overrides: Pattern rules (all-experts-weak, Q2-spike, low-acc exclusion)")
L.append("  Blend: Linear interpolation [20%-95% acc] -> [25%-85% expert weight]")
L.append("")
...
for r in results:
    ...
    L.append(f"    Q2/Q1 Ratio FC:  {r['ratio_fc']:>8,}    |  [validation] MA4:     {r['ma4_fc']:>8,}")
    L.append(f"    YoY Q2 FC:       {r['yoy_fc']:>8,}    |  [validation] BD FC:   {r['bd_fc']:>8,}")
    L.append(f"    Q2 Seasonal Avg: {r['q2_avg_fc']:>8,}    |  [validation] SCMS BU: {str(r['scms_fc']):>8}")
